...
/// <summary>
/// Provides extension methods for configuring compact structured logging with Serilog,
/// request/response body capture, excluded-path suppression, and a built-in log viewer.
/// </summary>
public static partial class LoggingExtensions
{
    private static string _logFilePrefix = "app";
    private static string _logDirectory = Path.Combine(Directory.GetCurrentDirectory(), "logs");
    private static string[] _excludedPaths = new[] { "/logs", "/health", "/alive" };
    ...
    {
        _logFilePrefix = serviceName.ToLowerInvariant()
            .Replace(' ', '-')
            .Replace('_', '-');
        ...
        var logPath = Path.Combine(_logDirectory, _logFilePrefix + "-.log");
        ...
        Log.Logger = new LoggerConfiguration()
            ...
            .Filter.ByExcluding(e => e.Properties.ContainsKey(ExcludedPathProperty))
    ...
    /// <param name="excludedPaths">Path prefixes to suppress. When <c>null</c>, uses the paths configured in <see cref="AddCompactLogging"/>.</param>
    public static IApplicationBuilder UseCompactRequestLogging(this IApplicationBuilder app, string[]? excludedPaths = null)
    {
        var paths = excludedPaths ?? _excludedPaths;
    ...
    /// <example>
    /// <para><b>Selective middleware — suppression + Serilog summary only (no body capture):</b></para>
    /// <code>
    /// app.UseExcludedPathLogging();
    /// app.UseSerilogCompactRequestLogging();
    /// </code>
    /// </example>
    /// <example>
    /// <para><b>With custom excluded paths:</b></para>
    /// <code>
    /// app.UseExcludedPathLogging(["/health", "/ready"]);
    /// </code>
    /// </example>
    /// </summary>
    /// <param name="app">The application builder.</param>
    ...
    /// <example>
    /// <para><b>All middleware with custom ordering:</b></para>
    /// <code>
    /// app.UseExcludedPathLogging();
    /// app.UseRequestResponseLogging();
    /// app.UseSerilogCompactRequestLogging();
    ...
    /// <example>
    /// <para><b>Serilog summary only (no body capture, no path suppression):</b></para>
    /// <code>
    /// app.UseSerilogCompactRequestLogging();
    /// </code>
    /// </example>
    ...
    /// <para><b>With custom excluded paths:</b></para>
    /// <code>
    /// app.UseSerilogCompactRequestLogging(["/health", "/ready"]);
    /// </code>
    /// </example>
    /// </summary>
    /// <param name="app">The application builder.</param>
    ...
    public static IEndpointRouteBuilder MapLogViewer(this IEndpointRouteBuilder app, string basePath = "/logs")
    {
        ...
        app.MapGet(basePath + "/api/entries", (string? file, string? level, string? search, int? page, int? pageSize) =>
        {
            ...
            var lines = ReadAllLines(targetFile);

            var filtered = lines
                .Select(ParseJsonLogLine)
                .Where(e => e != null)
        ...
        // --- UI ---
        app.MapGet(basePath, () => Results.Content(GetLogViewerHtml(), "text/html"));
    ...
    private static LogEntry? ParseJsonLogLine(string line)
    {
    ...
    /// <summary>
    /// Renders a Serilog message template by substituting property values.
    /// Tokens with a format specifier (e.g. {Elapsed:0.0000}) consume from the @r renderings array.
    /// </summary>
        ...
        <body>
            ...
            <script>
                ...
                function buildRow(l, i) {
                    ...
                    if (l.elapsed != null) {
                        const ms  = l.elapsed;
                        const cls = ms > 2000 ? ' very-slow' : µms > 500 ? ' slow' : '';
